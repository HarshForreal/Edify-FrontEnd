...
export default function InstructorDashboard() {
  ...
  const getInstructorCourses = async () => {
    try {
      ...
      if (Array.isArray(response)) {
        setCourses(response); // Ensure the response is an array
      } else {
  ...
  const deleteCourse = async (id) => {
    try {
      ...
      setCourses(courses.filter((course) => course.id !== id)); // Filter out the deleted course
  ...
  useEffect(() => {
    if (activePage === "courses") {
      getInstructorCourses(); // Fetch courses when the page loads or activePage changes
    }
  }, [activePage]);
